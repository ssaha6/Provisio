...
/*  ----  */
{
    ...
    */
	...
	while ((o = ProcessOption(Argc, Argv, "f+bpv+t+sm+c+S+I+ru+egX+whz+")))
    {
	if ( FirstTime )
	{
	...
	//Changed: Shambo: added error threshold option
	case 'z' :  SetIOpt(ERROR_THRESHOLD);
		//fprintf(Of, T_OptMinCases, ERROR_THRESHOLD);
		Check(ERROR_THRESHOLD, 0.0, 100.0);
		break; 
...
//**Shambo: pause output in VS before disappearing
// #ifdef DEBUG
// 	getchar();
// #endif
