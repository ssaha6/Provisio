...
//**Shambo: formtree creates the tree, so we need to tweak here.
...
/*   --------  */
{
    ...
    *Result = Node =
	...
	//**Shambo: undo pranavs modification: exact classification

	//original: 
	//if ( ClassFreq[BestLeaf] >= 0.999 * Cases  ||

	//Pranavs modification: Return only if there are no errors; otherwise split further. 
	//if ( ClassFreq[BestLeaf] >= Cases ||


	//**Shambo: if flase is majority by some threshold, 
	// then classify some true as false, thus introducing errors
	if ((ClassFreq[BestLeaf] >= ((float)ERROR_THRESHOLD/(float)100.0) * Cases && strcmp(ClassName[BestLeaf], "false") == 0) ||
		//true needs to be 100% to form a leaf, as false wont change to true. 
		(ClassFreq[BestLeaf] >= 0.999 * Cases && strcmp(ClassName[BestLeaf], "true") == 0) || 
		ClassFreq[BestLeaf] >= Cases ||
		Cases < 2 * MINITEMS ||
		MaxLeaves < 2)
	{
		
	//**Shambo: print class name for bestleaf
	//fprintf(Of, "\tbest attribute %s", ClassName[BestLeaf]);
